# EC-1062 — Synthetic Data Generation Strategy

**Status:** for review. No data generated yet.
**Format:** Option B — flat CSVs matching the extraction query's 80 columns exactly. Drop-in replacement for `data/raw/expense_claims.csv`; existing notebook and `src/` run unchanged.
...
Archetypes are assigned per **(employee, expense_type)** — finer than category, so that a person can be a steady `Local Cab` claimant *and* a rare `Airfare` claimant within the same Travel category. That combination is precisely what the grain test needs.
